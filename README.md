...
9. Класс BasketItemUI

Класс наследуемый от абстрактного класса ProductUI для отображения товаров в корзине.
Метод:

- Метод index является главным инструментом для пронумеровывания товаров в корзине. Функция action принимает на вход конструктор, который в свою очередь получает имя стиля компонента и контейнер шаблона (HTMLElement) при нажатии кнопки.
